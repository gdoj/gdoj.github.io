...
function init_submission_table(
  $,
  submission_activity,
  metadata,
  language_code
  ...
  function draw_contribution(year) {
    ...
    $div
      .find("#prev-year-action")
      .css("display", year > (metadata.min_year || current_year) ? "" : "none");
    $div
      .find("#next-year-action")
      .css("display", year < current_year ? "" : "none");
    ...
    var sum_activity = days.reduce(function (sum, obj) {
      return sum + obj.activity;
    }, 0);
    $div
      .find("#submission-total-count")
      .text(
        ngettext(
          "%(cnt)d total submission",
          "%(cnt)d total submissions",
          sum_activity
    ...
    for (
      ...
      $div
        .find("#submission-" + current_weekday)
        .append($("<td>").addClass("activity-blank").append("<div>"));
    }

    var max_activity = Math.max(
      1,
      Math.max.apply(
        null,
        days.map(function (obj) {
          return obj.activity;
        })
      )
    );
    days.forEach(function (obj) {
      var level = Math.ceil(
        (obj.activity / max_activity) * (activity_levels - 1)
      );
      var text = ngettext(
        ...
        .replace(
          "%(date)s",
          obj.date.toLocaleDateString(language_code, {
            month: "short",
            year: "numeric",
            day: "numeric",
          })
