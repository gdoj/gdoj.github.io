...
$(function () {
  ...
  $category
    ...
    .change(clean_submit);
  $("#types")
    .select2({ theme: "dmoj", multiple: 1, placeholder: "Lọc theo dạng..." })
    .css({ visibility: "visible" });
  ...
  $("#random").click(function (e) {
    ...
    $form.attr("action", "/problems/random/").attr("target", "_blank").submit();
    $form.attr("action", action).attr("target", "");
    e.preventDefault();
  });
  $("#go").click(clean_submit);
  $(
    "input#full_text, input#hide_solved, input#show_types, input#has_public_editorial"
  ).click(function () {
    prep_form();
    $("<form>")
