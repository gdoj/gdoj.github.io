 ...
 */ (function (global, factory) {
  ...
  helpers$1.extend(DatasetController.prototype, {
    ...
    createMetaData: function (index) {
      ...
      return (
        type &&
        new type({ _chart: me.chart, _datasetIndex: me.index, _index: index })
      );
    },
  ...
  var platform_dom$1 = Object.freeze({ default: platform_dom });
  ...
  helpers$1.extend(Chart.prototype, {
    ...
    drawDataset: function (index, easingValue) {
      var me = this;
      var meta = me.getDatasetMeta(index);
      var args = { meta: meta, index: index, easingValue: easingValue };
  ...
  var core_scale = core_element.extend({
    ...
    update: function (maxWidth, maxHeight, margins) {
      ...
      for (i = 0, ilen = labels.length; i < ilen; ++i) {
        ...
        tick = ticks[i];
        if (!tick) {
          ticks.push((tick = { label: label, major: false }));
        } else {
          tick.label = label;
        }
      }
      ...
      if (opts.stacked || hasStacks) {
        ...
        helpers$1.each(datasets, function (dataset, datasetIndex) {
          ...
          if (valuesPerStack[key] === undefined) {
            valuesPerStack[key] = { positiveValues: [], negativeValues: [] };
          }
  ...
  var INTERVALS = {
    ...
    second: { common: true, size: 1000, steps: [1, 2, 5, 10, 15, 30] },
    minute: { common: true, size: 60000, steps: [1, 2, 5, 10, 15, 30] },
    hour: { common: true, size: 3600000, steps: [1, 2, 3, 6, 12] },
    day: { common: true, size: 86400000, steps: [1, 2, 5] },
    week: { common: false, size: 604800000, steps: [1, 2, 3, 4] },
    month: { common: true, size: 2.628e9, steps: [1, 2, 3] },
    quarter: { common: false, size: 7.884e9, steps: [1, 2, 3, 4] },
...
$(function () {
  ...
  function idAndTextMatcher(params, data) {
    ...
    return data.text.search(regex) >= 0 || data.id.search(regex) >= 0
      ? data
      : null;
  }
  $("#status")
...
function stats_graph(raw_data) {
  ...
  chart = new Chart(ctx, {
    ...
    options: {
      ...
      tooltips: { titleFontFamily: font, bodyFontFamily: font },
...
function load_dynamic_update() {
  ...
  var organization_filter = $.map(
    $("select#organization option[selected]"),
    _collect
  );
  ...
  var update_submission = function (message, force) {
    ...
    if (row.length < 1) {
      if (id < first) return;
      first = id;
      row = $("<div>", { id: id, class: "submission-row" })
        .hide()
        .prependTo(table);
      if (table.find(">div").length >= 50)
  ...
  var stats_update_interval = 0;
  function update_stats() {
    if (Date.now() - last_stat_update < stats_update_interval) return;
    $.ajax({ url: "?results" })
      .done(function (data) {
  ...
  $(window).on("dmoj:window-visible", function () {
    if (stats_outdated) update_stats();
  });
  var $body = $(document.body);
  event_dispatcher.on("submissions", function (message) {
    if (
      (dynamic_user_id && message.user != dynamic_user_id) ||
    ...
    if (message.type == "update-submission") {
      ...
      if ($("body").hasClass("window-hidden")) return (stats_outdated = true);
      update_stats();
    }
  });
