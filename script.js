...
function count_down(label) {
    ...
    var timer = setInterval(function() {
        ...
        var s = time % 60;
        if (d > 0)
            label.text(npgettext('time format with day', '%d day %h:%m:%s', '%d days %h:%m:%s', d).replace('%d', d).replace('%h', format(h)).replace('%m', format(m)).replace('%s', format(s)));
        else
            label.text(pgettext('time format without day', '%h:%m:%s').replace('%h', format(h)).replace('%m', format(m)).replace('%s', format(s)));
    }, 1000);
}
...
window.notify = function(type, title, data, timeout) {
    if (localStorage[type + '_notification'] != 'true')
        return;
    ...
    function set_filters() {
        if (window.WebSocket) {
            filter_timeout = setTimeout(function() {
                if (receiver.websocket && receiver.websocket.readyState === WebSocket.OPEN && receiver.websocket.readyForData === true) {
                    receiver.websocket.send(JSON.stringify({
                        command: 'set-filter',
